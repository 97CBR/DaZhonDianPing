...
import lxml
from DaZhonDianPing.WebContent import get_content, get_css, get_fu_content
import xlwt
import xlrd
class changeSentence:
    ...
    def get_string(self):
        for info in range(0,515):
            ...
            # print(values[-1])
            self.string = values[9]
            self.box_class = []
            self.box_class_my=[]
            try:
                for i in range(0, len(self.string) - 1):
                    if self.string[i] + self.string[i + 1] == 'fu':
                        # print(self.string[i:i + 6:])
                        self.box_class.append(self.string[i:i + 7:])
                for i in range(0, len(self.string) - 1):
                    if self.string[i] + self.string[i + 1] == 'my':
                        # print(self.string[i:i + 6:])
                        self.box_class.append(self.string[i:i + 7:])
            except:
                ...
    ...
    def run_it(self):
        try:
            ...
            for info in content:
                ...
                # print(position)
                new = get_fu_content().show(cx=position[0], cy=position[1])
                # print(new)
                rs = '{}'.format(c)
                # print(rs)
                self.string = self.string.replace(rs, new)

                my_content = self.box_class_my
                for info in my_content:
                    # print(info)
                    c = str(info)
                    p = self.css.copy()
                    # print(c[0])
